...
class OdometryMotionModel(Node):
  ...
  def __init__(self):
    ...
    self.odom_sub_ = self.create_subscription(Odometry, "amr_controller/odom", self.odomCallback, 10)
    ...
    def odomCallback(self, odom):
        ...
        for sample in self.samples.poses:
            ...
            sample_roll, saple_pitch, sample_yaw = euler_from_quaternion(sample_q)
        ...
        self.last_odom_x = odom.pose.pose.position.x
        self.last_odom_y = odom.pose.pose.position.x
        self.last_odom_theta = yaw
